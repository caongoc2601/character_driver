...
static ssize_t vchar_driver_read(struct file *filp, char __user *user_buf, size_t len, loff_t *off)
{
	...
	int num_bytes = 0;
	printk(KERN_INFO "Handle read start from %lld, %zu bytes\n", *off, len);
...
static void handle_timer(unsigned long data)
{
	...
	/* Neu muon kernel timer tiep
	 * tuc hoat dong thi can thay
	 * doi thoi diem bao thuc*/
...
static int __init vchar_driver_init(void)
{
	...
	/*khoi tao, cau hinh va dang ky kernel timer voi linux kernel*/
